...
static void __on_user_offline(connection_id_t conn_id, uint32_t uid, int reason)
{
  ...
  // If the AI agent (uid=1001) has left, clear the flag
  if (uid == CONVO_AGENT_RTC_UID) {
    printf("⚠️  AI Agent (uid=%lu) has left RTC channel\n", uid);
    printf("   Clearing agent state flags\n");
                            ...
                            const audio_frame_info_t *info_ptr)
{
  ...
  if (audio_count++ % 100 == 0) {  // Print every 100 packets to avoid spam
